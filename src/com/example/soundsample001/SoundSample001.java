...
class MyView extends View {
	...
	*/
	...
	//        event.getActionMasked() //API8
		if (( event.getAction() & MotionEvent.ACTION_MASK ) == MotionEvent.ACTION_DOWN ) {  //API8以前(API5~7)でもOK
		           // ACTION_DOWNは指が触れた時
				   // 音の再生開始位置を0ミリセカンドの位置に設定する
			       mp.seekTo( 0 );
			      // 音の再生を開始する
		     	  mp.start();
		} else if (( event.getAction() & MotionEvent.ACTION_MASK ) == MotionEvent.ACTION_UP ) { //API8以前(API5~7)でもOK
		          // ACTION_UPは指が離れた時
				  // 音を停止する
			mp.stop();
